...
# Goal: Compare AFINN vs Bing vs NRC per stop (clean lines, no ribbons), with location labels.
# Input :
...
need <- c("order","location","val_afinn_adj","val_bing_adj","val_nrc_adj")
stopifnot(all(need %in% names(tok)))

# reshape to long and keep per-lexicon scored tokens
long <- tidyr::pivot_longer(
  tok,
  cols = c(val_afinn_adj, val_bing_adj, val_nrc_adj),
...
summ_lex <- long |>
  ...
  dplyr::summarise(
    ...
    mean_val_lex_scored  = ifelse(tokens_scored_lex > 0, sum(val[has_lex]) / tokens_scored_lex, NA_real_),
    .groups = "drop"
  ) |>
...
summ_lex <- dplyr::left_join(summ_lex, boot_tbl, by = c("order","location","lexicon"))
...
# colours
pal <- c("AFINN" = "#2563eb", "Bing" = "#f59e0b", "NRC" = "#10b981")
...
plt_compare <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    subtitle = "Clean lines (no CI ribbons) for clearer trend comparison"
  ) +
...
plt_facet <- ggplot2::ggplot(
  ...
  ggplot2::labs(x = "Journey order", y = "Mean valence (scored tokens)",
                title = "Lexicon-Specific Trends (clean lines)") +
