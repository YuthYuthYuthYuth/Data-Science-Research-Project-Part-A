# R/12_geo_map.R
# Goal: Plot the itinerary on a world map.
# Inputs :
...
#   outputs/figs/journey_route.pdf
...
# coordinate lookup
# You can edit these or provide data/raw/stop_coords.csv to override.
coords_default <- tibble::tibble(
  ...
  lat = c(
    51.5074,   # London
    29.9668,   # Suez (use as proxy for "Suez/Aden")
...
if (length(missing) > 0) {
  message("No coordinates for: ", paste(missing, collapse = ", "),
...
# plot
# x/y limits tuned to keep the journey centered (adjust if you prefer)
xlim <- c(-140, 150)
ylim <- c(-5, 65)

base_map <- ggplot2::ggplot() +
  ggplot2::geom_sf(data = world, fill = "#f3f4f6", colour = "white", linewidth = 0.2) +
  ggplot2::coord_sf(xlim = xlim, ylim = ylim, expand = FALSE) +
  ggplot2::theme_void(base_size = 11) +
  ggplot2::theme(panel.background = ggplot2::element_rect(fill = "white", colour = NA))

plt_route <- base_map +
  ggplot2::geom_sf(data = route, colour = "#2563eb", linewidth = 1.1, alpha = 0.9) +
  ggplot2::geom_sf(data = pts, colour = "white", fill = "#ef4444", size = 2.6, shape = 21, stroke = 0.3) +
  # order labels
  {
    if (requireNamespace("ggrepel", quietly = TRUE)) {
      ggrepel::geom_text_repel(
        ...
        size = 3, seed = 2025, point.padding = 0.2, max.overlaps = 100
      )
    } else {
      ggplot2::geom_text(
        data = stops_map,
  ...
  ggplot2::labs(title = "Route of Phileas Fogg’s Journey",
...
ggplot2::ggsave(file.path(outdir, "journey_route.png"), plt_route,
                width = 9, height = 5.2, dpi = 300)
ggplot2::ggsave(file.path(outdir, "journey_route.pdf"), plt_route,
                width = 9, height = 5.2, device = cairo_pdf)

# Also write out the stops used (handy for appendix)
readr::write_csv(stops, here::here("outputs/figs/stop_key_with_coords.csv"))
