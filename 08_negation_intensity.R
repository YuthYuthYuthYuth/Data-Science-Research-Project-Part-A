...
# Required columns
req_val_cols <- c("lex_AFINN","lex_Bing","lex_NRC","word","sent_id","token_id")
...
score_tokens <- function(df, id_cols) {
  ...
  df <- df %>%
    ...
    dplyr::mutate(
      ...
      neg_w  = dplyr::lag(cneg,  1, default = 0) - dplyr::lag(cneg,  window_k + 1, default = 0),
      amp_w  = dplyr::lag(camp,  1, default = 0) - dplyr::lag(camp,  window_k + 1, default = 0),
      down_w = dplyr::lag(cdown, 1, default = 0) - dplyr::lag(cdown, window_k + 1, default = 0)
    ) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(
  ...
  # Base valence (scale AFINN to ±1 to match others)
  df <- df %>%
    dplyr::mutate(
      val_afinn = lex_AFINN / 5,
      val_bing  = lex_Bing,
      val_nrc   = lex_NRC
    )
  ...
  df <- df %>%
    ...
    dplyr::mutate(
      val_base = {
        v <- c(val_afinn, val_bing, val_nrc)
        nz <- v[v != 0]
  ...
  df <- df %>%
    dplyr::mutate(
      ...
      has_valence   = (val_afinn != 0) | (val_bing != 0) | (val_nrc != 0)
    )
...
stopifnot(all(scored_stop$val_base_adj <=  5))  # sanity bounds (soft)
